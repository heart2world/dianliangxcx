...
class SettingController extends AdminbaseController{
	...
	// 电量提醒
	public function dldoing()
	{
		$cmf_settings=$this->options_model->where("option_name='dl_settings'")->getField("option_value");
		$this->assign("info",json_decode($cmf_settings,true));
		$this->display();
	}
	public function dldoingpost()
	{
		if(IS_POST)
		{
			$value =I('lessdianliang');
			$data['option_name']="dl_settings";
			$options['lessdianliang'] =$value;
			$data['option_value']=json_encode($options);
			if($this->options_model->where("option_name='dl_settings'")->find()){
				$result=$this->options_model->where("option_name='dl_settings'")->save($data);
			}else{
				$result=$this->options_model->add($data);
			}
