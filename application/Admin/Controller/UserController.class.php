...
class UserController extends AdminbaseController{
	...
	public function index(){
		...
		foreach ($users as $key => $value) {
			$users[$key]['rolename'] = D()->table(C('DB_PREFIX')."role r,".C('DB_PREFIX')."role_user ru")->where("r.id=ru.role_id and ru.user_id='".$value['id']."'")->getField('r.name');
			$users[$key]['schoolname'] =M('school')->where("id='".$value['sid']."'")->getField('schoolname');
		}
		...
		if(IS_POST){
			if(!empty($_POST['role_id'])){
				...
				if ($this->users_model->create()!==false) {
					...
					if(!preg_match('/^1[345789]{1}\d{9}$/',trim($_POST['user_login'])))
					{
						$this->error("手机号格式有误");
					}
	...
	public function detail()
	{
		...
		$user=$this->users_model->where(array("id"=>$id))->find();
		$role_ids =M('role_user')->where("user_id='".$user['id']."'")->getField('role_id');
		$user['rolename'] =M('role')->where("id='$role_ids'")->getField('name');
